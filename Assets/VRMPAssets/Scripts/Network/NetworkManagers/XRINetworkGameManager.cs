    ...
    public class XRINetworkGameManager : NetworkBehaviour
    {
        ...
        protected virtual bool ConnectedToLobby()
        {
            ...
            if (autoConnectOnLobbyJoin)
            {
                ...
                {
                    Utils.Log($"{k_DebugPrepend}Starting as Host");
                    connected = NetworkManager.Singleton.StartHost();
                }
                else
                {
                    Utils.Log($"{k_DebugPrepend}Starting as Client");
        ...
        {
            Utils.Log($"{k_DebugPrepend}Connection approval requested for client {request.ClientNetworkId}");

            response.Approved = true;
            response.CreatePlayerObject = true;
            response.Pending = false;

            Utils.Log($"{k_DebugPrepend}Connection approved for client {request.ClientNetworkId}");
        }
